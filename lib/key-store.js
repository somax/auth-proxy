...
const DEFAULT_LIFE_TIME = 300000;
...
KeyStore.prototype.insert = function insert(key, _lifeTime) {
    let _expTime = Date.now() + (_lifeTime || this.lifeTime);
    let _key = `${key}.${_expTime}`
...
KeyStore.prototype.sweep = function sweep() {
    ...
    for (var i = 0; i < this.storage.length; ) {
        ...
        log.debug('过期时间：', keyExp, '< 现在：', now, keyExp < now ? '已过期' : '未过期')
        if (keyExp >= now) {
            return;
        }
        this.storage.shift();
